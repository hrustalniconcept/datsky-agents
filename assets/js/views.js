...
(function () {
  ...
  function home() {
    ...
    return sec(
      ...
      '<h1>Сюда приезжают смотреть квартиру — влюбляются в место</h1>' +
      '<p class="lead lead--big">Через месяц после переезда человек живёт иначе. Вечером выходит во двор просто так, а не по делу, и знает соседей по именам. Лес в ста метрах, залив в пятистах, магазин и садик пешком — выбирать между природой и городскими удобствами здесь не приходится.</p>' +
      '<p class="lead">Уговаривать не придётся: место и соседи справляются сами. От вас другое — привезти и подобрать то самое: участок под окнами, террасу или планировку, в которой поместится вся семья. ' + r.vsego_lotov + ' квартир с ключами, комиссия 4,5% — до ' + nf(Math.round(maxCom / 1000)) + ' тысяч с одной сделки.</p>' +
