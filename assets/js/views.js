...
(function () {
  ...
  function home() {
    ...
    var tasks = [
      ...
      ['Подготовиться к показу', 'Семь типов покупателей, маршрут показа по поясам, одиннадцать возражений с дословными ответами.', '#/client', 'Что говорить'],
      ['Посчитать деньги', 'Ваша комиссия по каждому лоту, четыре сценария покупки, точка равенства на 27-м месяце.', '#/terms', 'Условия и расчёты'],
    ...
    return sec(
      ...
      '<p class="lead">«Хрустальный парк» — девять кварталов на Байкальском тракте, каждый в архитектуре своей страны, больше трёх тысяч жителей. Датский сдан в декабре 2025-го: дома заселены, деревья растут, во дворах гуляют дети. Клиент приезжает не на стройплощадку с рендером на заборе, а в живой микрорайон.</p>' +
      '<p class="lead">Здесь Масленица с блинами, Пасхальный квест пятый год подряд, детская ярмарка в конце августа и один общий новогодний салют на весь микрорайон. Больше тридцати событий в год, и половину придумывают сами жители. Магазин, пекарня, кафе, ФАП, аптека и садик — пешком. До сорока с лишним секций Байкал-Арены — четырнадцать минут. Лес в ста метрах, залив примерно в пятистах.</p>' +
      '<p class="lead">В вашем списке жилых комплексов второго такого нет, и это ваш аргумент. Здесь не нужно уговаривать: покажите двор без машин, террасу на закате и соседей, которые здороваются. Через год клиент возвращается с рекомендацией, а не с претензией. Комиссия — 4,5% от цены договора, закрепление на 60 дней, персональный менеджер ведёт сделку вместе с вами.</p>' +
    ...
    sec(
      eyebrow('Что показывать первым') +
      '<h2>Здесь уже сложилось соседство. Это не достраивается</h2>' +
      '<p class="lead">Двор и фасады конкурент повторит за сезон. Соседей, традиции и чат, в котором вечером просят колпак для детской фотосессии, а наутро возвращают с шоколадкой, — не повторит никто. Это единственная часть продукта, которую невозможно скопировать, и показывать её клиенту стоит раньше планировок.</p>' +
      '<div class="grid g4">' +
      '<div class="card"><h4>Больше 30 событий в год</h4><p class="small" style="margin:0">Масленица, Пасхальный квест пятый год подряд, День здоровья, фестиваль детских талантов, ХП-ФЕСТ на 250 гостей, детская ярмарка, Хэллоуин, общий салют. Половину придумывают сами жители.</p></div>' +
      '<div class="card"><h4>Двор без машин</h4><p class="small" style="margin:0">Парковки вынесены по периметру, коэффициент 1,6 на квартиру — вдвое выше городского. Ребёнка отпускают гулять одного, и это здесь обычное дело.</p></div>' +
      '<div class="card"><h4>Девять кварталов вместо одного двора</h4><p class="small" style="margin:0">Французский, Итальянский, Испанский, Чешский, Финский, Австрийский, Vila и Датский. Гуляют по всем сразу, а не по своему периметру. Озеленение — больше половины территории.</p></div>' +
      '<div class="card"><h4>Всё нужное пешком</h4><p class="small" style="margin:0">Магазины, пекарня с рейтингом 4,8, кафе, ФАП, аптека, пункты выдачи, частный садик и лыжная мастерская. Городской набор повседневных дел закрывается без машины.</p></div>' +
      '</div>' +
      '<div class="actions"><a class="btn btn--ghost" href="#/life">Год в квартале и соседи</a>' +
    ...
    sec(
      eyebrow('Аргументы для показа') +
      '<h2>Шесть фактов, которые клиент проверит сам</h2>' +
      '<p class="lead">Рулеткой и навигатором. Проверит — и поверит всему остальному, что вы говорите.</p>' +
    ...
    (function () {
      ...
      return sec(
        ...
        '<p class="small">Кафе рядом меньше — признаём сразу. Но до Байкал-Арены отсюда 14 минут и два светофора, а из городского комплекса в четырёх километрах — 11 минут и пять. Плюс кухня-гостиная 34–42 метра, возможность двух санузлов уже с 63 квадратов и место, где реально работать из дома.</p>' +
  ...
  function client() {
    ...
    sec(
      ...
      '<p class="lead">' + esc(M.princip) + ' Ведите не по квартирам, а по поясам: сначала среда, потом дом, и только в конце — терраса.</p>' +
  ...
  function life() {
    var L = d().life;
    var pics = commonBy('lyudi').concat(commonBy('aero', 2)).concat(commonBy('dvor', 3)).concat(commonBy('priroda', 2));
